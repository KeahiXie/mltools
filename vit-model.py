...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        assert x.shape[1] == self.patcher.in_channels, "Input channels mismatch."
...
class TransformerEncoderBlock(nn.Module):
    ...
    def forward(self, x):
        x_res = x
        x = self.msa_block(x) + x_res

        x_res = x
        x = self.mlp_block(x) + x_res
...
class ViT(nn.Module):
    def __init__(self, img_size: int = 224, in_channels: int = 3, patch_size: int = 16, num_transformer_layers: int = 12,
                 ...
                 mlp_dropout: float = 0.1, embedding_dropout: float = 0.1, num_classes: int = 1000):
        super().__init__()
